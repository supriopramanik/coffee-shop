...
export default function Home(){
  return (
    <div>
      ...
      {/* removed extra carousel section for cleaner layout */}
...
function FeaturedProducts(){
  ...
  const pageCountRaw = items.length && perPage ? Math.ceil(items.length / perPage) : 0;
  const pageCount = pageCountRaw > 0 ? pageCountRaw : 1;

  // keep page in range when data/perPage changes
  useEffect(()=>{
    if(page >= pageCount){
      setPage(0);
    }
  ...
  const visibleCount = Math.min(perPage, items.length);
  const visibleItems = [];
      ...
      >
        {visibleItems.map(p => (
          <div className="card" key={p._id}>
            <img src={p.imageUrl} alt={p.name} className="responsive" style={{borderRadius:12,marginBottom:8}}/>
            <h3>{p.name}</h3>
            <p>৳ {p.price}</p>
            <Link to="/shop" className="btn">Add to Cart</Link>
          </div>
        ))}
      </div>
      {pageCount > 1 && (
        <div style={{display:'flex',alignItems:'center',justifyContent:'center',gap:8,marginTop:12}}>
          ...
          <div style={{display:'flex',gap:6,flexWrap:'wrap',maxWidth:260,justifyContent:'center'}}>
            {Array.from({length: pageCount}).map((_,i)=>(
              <button
                ...
                style={{
                  ...
                  border:'none',
                  padding:0,
                  background: i===page ? '#6b3e2e' : '#d3c3b5',
                  cursor:'pointer'
                }}
              />
            ))}
